...
Compose waits for the PostgreSQL healthcheck, runs the one-shot `migrate` service with `up`, runs the [seed service](../seeds/README.md), then starts the backend only after both services succeed. A migration or seed failure blocks backend startup. Migrations are not run by the Go application.
