...
6. Apply a confirmed pattern only when it matches the current context. Use these scope rules:
   - Tests: start with `go test ./path/to/package` or `go test ./path/to/package -run '^TestName$'`, replacing placeholders with existing names. For the current handler package, use `go test ./internal/handler`. Verify that a targeted test actually ran; a successful result with no matching tests is not validation. Expand to `go test ./...` when affected scope or required project checks justify it.
   - Build/run: select the required package with `go build ./path/to/package` or `go run ./path/to/main`. For the backend entry point, use `go build ./cmd/api` or `go run ./cmd/api`. Check output paths for builds and configuration, ports, and service side effects before starting the application.
